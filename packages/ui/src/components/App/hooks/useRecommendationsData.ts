...
export const generateMovie = ({
  title, description, imageHref, rating,
}) => ({
  title,
  description,
  imageHref,
...
export const fakeData = [
  {
    title: { color: '#58DB85', text: 'Recommended' },
    items,
  },
  {
    slideHeight: 300,
    slideWidth: 300,
    title: { color: '#FF6D6D', text: 'Most Popular' },
    items: [
      generateMovie({
        title: 'The Midnight Sky',
        description:
  ...
  {
    title: { color: '#FFBF60', text: 'More like "Dune"' },
    items: [].concat(items).reverse(),
    slideWidth: 400,
    slideHeight: 400,
  },
  {
    title: { color: '#60e2ff', text: 'More like "Interstellar"' },
    items,
  },
  {
    title: { color: '#58DB85', text: 'Recommended' },
...
const useRecommendationsData = (): typeof fakeData => {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      //   }))();
      // // @ts-ignore
      // dispatch(setRecommendations(fetchResponse));
    };
